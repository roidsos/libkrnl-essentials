...
struct tar_contents parse_tar(struct tar_header *header,__attribute__((unused)) uint64_t file_size) {
    vector_init(&headers);
    ...
    while (true) {
        ...
        _header =(struct tar_header *)((uint64_t)_header + ((size / 512) + 1) * 512);
